...
<div class="w-full flex justify-center items-center rounded-3xl">
  ...
  <div id="modal-container" class="w-full flex flex-col justify-start items-center relative bg-white nunito- p-2 rounded-3xl bg-gradient-to-r from-[#184E77] to-[#52B69A]">
    <div class="w-full flex flex-col justify-start items-center bg-white p-8 rounded-3xl space-y-8">
      ...
      <form action="{{ route('expenses.update', $expense->id) }}" method="POST" class="w-full mx-auto p-6 "  enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <label for="expense_type" class="block text-xl text-black font-bold">Expense Type</label>
            <input
              type="text"
              id="expense_type"
              name="expense_type"
              placeholder="Enter incident type"
              value="{{ old('expense_type', $expense->category) }}"
          ...
          <div>
              <label for="expense_date" class="block text-xl text-black font-bold">Expense Date:</label>
              <input
                  type="date"
                  id="expense_date"
                  name="expense_date"
                  placeholder="DD:MM:YYYY"
                  value="{{ old('expense_date', $expense->submitted_date) }}"
          ...
          <div>
            <label for="expense_status" class="block text-xl text-black font-bold">Resolution Status:</label>
            <input
              type="text"
              id="expense_status"
              name="expense_status"
              placeholder="Enter the status"
              value="{{ old('expense_status', $expense->status) }}"
          ...
          <div>
            ...
            <input
              ...
              value="{{ old('approved_by', $expense->approved_by) }}"
          ...
          <div>
            ...
            <div id="file-list" class="text-black text-sm w-full bg-[#D9D9D980] flex flex-col justify-start items-start rounded-b-xl pr-4 pt-4">
              ...
              <ul id="file-list-items" class="w-full px-4">
                  @php
                      $documents = json_decode($expense->supporting_documents, true) ?: [];
...
function loadDescription(description) {
  ...
  const dynamicDescription = "{{ $expense->description }}";
